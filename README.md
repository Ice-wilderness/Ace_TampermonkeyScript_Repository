...
- `scripts/`：用户脚本源码，每个 `.js` 脚本独立维护。
- `tools/userscript/`：本地调试、Playwright 注入、页面采集等开发工具。
- `tests/userscript/`：用户脚本自动化测试和离线 HTML fixture。
- `artifacts/`、`test-results/`、`.browser-profiles/`：本地测试产物和浏览器配置目录，不纳入版本管理。
